...
- **Review gate (multica-style)** — the board is a shared human+agent board (`status` = columns: Backlog → In-progress → **Review** → Done; agents are teammates via an `assignee` field). When a requirement goes green and opens a PR, it stops at a **Review card** (diff stat + checker verdicts + summary + PR link) where you **Approve / Request changes / Hold** — approval marks it Done; requested changes are saved as a child record and bounce it back to In-progress.
