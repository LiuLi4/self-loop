...
| `SELF_LOOP_SCOPE_RULE` | 可选，范围约束（自然语言）。设了才做边界守卫，越界需求只标 待澄清 不实现 | 否 |
...
Intake 把 issue 写进看板后，对其中**需要用户回答/澄清**的（`type=待澄清`，或 `阻塞`/`缺口` 里 evidence 标注"需外部输入"的），在启动 build 前用问答逐个解决——不要带着这些悬而未决就开跑：
...
3. 对每个得到回答的 issue，用 `issue-upsert` 幂等写回：
   - **建子记录**存答案：`external_key=<qkey>#answer`、`parent_key=<qkey>`、`requirement` 同父、`type` 同父、`status=已完成`、`assignee=用户`、`title=[答复] <原标题>`、`evidence=<用户原话答案>`、`updated_round=<当前轮>`；
   - 父 issue 置 `status=已完成`，evidence 追加"已由用户答复，见子记录 <qkey>#answer"。
   ```bash
...
把看板当成 人+agent 共用的看板，**状态即列**：待处理 → 进行中 → **待评审(Review)** → 已完成（另有 不做）；阻塞项 = `阻塞`型 + 待处理。agent 是"队友"：每条 issue 记 `assignee`（如 `实现-REQ1` / `校验` / `用户`），完成后主动进待评审、主动报阻塞。
...
3. **据答幂等写回看板**：
   - **Approve** → 该需求 `#intake` 置 `已完成`、`assignee=用户`；再单独问是否要执行 merge（外发动作，分开确认）。
   - **Request changes** → 用户意见写成**子记录**（`external_key=<key>#review-rN`、`parent_key=<key>`、`type=缺口`、`assignee=用户`、evidence=意见），父 `#intake` 退回 `进行中`，下一轮 build 带上反馈重做。
   - **Hold** → 留 `待评审`，记 evidence。
4. 多个需求同时到 Review 时，**一次性把多张卡一起呈现**（一个 AskUserQuestion 多道题），别让用户逐个等。

> 对应 multica：Review 列 + Approve/Request-changes 审批门；agent 当队友(assignee)、主动报阻塞；成功经验沉淀进 Rules `## Learned`（≈ multica 的 compound skills）。

## 4. 汇报

脚本返回 `{ rounds, converged, inScope, openIssues }`。据此汇报：收敛与否 / 跑了几轮 / 纳入几个范围内需求；看板剩余未关闭事项（`converged=false` 表示到 `maxRounds` 仍未全绿，附剩余事项交人工）；越界(待澄清)需求清单——**未**被实现，需人工决定是否立新 spec。

## 护栏（脚本已内置，汇报时也要守住，不得放宽）

- **范围守卫**：设了 `SELF_LOOP_SCOPE_RULE` 时，越界需求只标 `待澄清`，绝不自动实现或新建范围外 spec。
